...
from core.converter.converter import PandasConverter
...
class TestPandasConverter(TestCaseCompare):
    @classmethod
    def setUpClass(cls):
        super(TestPandasConverter, cls).setUpClass()
        cls.input_folder = cls.input_folder / 'core' / 'converter'
        cls.output_folder = cls.output_folder / 'core' / 'converter'

    def test_json_to_csv(self):
        out_f = self.output_folder / "test_json_to_csv_out.csv"
        PandasConverter().convert_file(self.input_folder / 'sample.json', 'csv', out_f)
    ...
    def test_json_to_csv_with_convert_options(self):
        ...
        PandasConverter().convert_file(input_file_path=self.input_folder / 'sample.json',
                                       output_format='csv',
    ...
    def test_json_to_ndjson(self):
        ...
        PandasConverter().convert_file(self.input_folder / 'sample.json', 'ndjson', out_f)
    ...
    def test_json_to_excel(self):
        ...
        PandasConverter().convert_file(self.input_folder / 'sample.json', 'xlsx',
                                       self.out_file[method_id])

    def test_ndjson_to_csv(self):
        out_f = self.output_folder / "test_ndjson_to_csv_out.csv"
        PandasConverter().convert_file(self.input_folder / 'sample.ndjson', 'csv', out_f)
        self.file_compare(out_f, self.input_folder / 'sample_with_index.csv')

    def test_ndjson_to_json(self):
        out_f = self.output_folder / "test_ndjson_to_json_out.json"
        PandasConverter().convert_file(self.input_folder / 'sample.ndjson', 'json', out_f)
    ...
    def test_csv_to_json(self):
        ...
        PandasConverter().convert_file(self.input_folder / 'sample.csv', 'json', out_f)
    ...
    def test_csv_to_json_with_load_options(self):
        ...
        PandasConverter().convert_file(self.input_folder / 'sample_semicolon.csv', 'json', out_f,
                                       load_options={'sep': ';'})
        self.assertCountEqual(json.load(out_f.open()), json.load((self.input_folder / 'sample_null.json').open()))

    def test_csv_to_ndjson(self):
        out_f = self.output_folder / "test_csv_to_ndjson_out.ndjson"
        PandasConverter().convert_file(self.input_folder / 'sample.csv', 'ndjson', out_f)
