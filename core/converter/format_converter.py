...
class FormatConverter(ABC):
    ...
    def validate_convert_file(self, input_file_path, output_format, output_file_path, input_format):
        if output_format not in self.allowed_formats:
            raise ValueError(f"Expected input format to be {str(self.allowed_formats)} instead of {output_format}")
        ...
        if input_format not in self.allowed_formats:
            raise ValueError(f"Expected input format to be {str(self.allowed_formats)} instead of {input_format}")
